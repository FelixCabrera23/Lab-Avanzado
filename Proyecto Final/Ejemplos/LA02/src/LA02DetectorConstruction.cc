...
G4VPhysicalVolume* LA02DetectorConstruction::Construct()
{
  ...
  G4double target_sizeX = 25*cm;
  G4double target_sizeY = 25*cm;
  G4double target_sizeZ = 10*cm;
  ...
  /********************************************************/
  // Provando una nueva configuración

  G4ThreeVector pos2 = G4ThreeVector(0, 0, 49*cm);

  G4double plate_sizeX = 25*cm;
  G4double plate_sizeY = 25*cm;
  G4double plate_sizeZ = 0.5*cm;
  G4Material* plate_mat = nist->FindOrBuildMaterial("G4_Pb");
  G4Box* solidPlate =
    new G4Box("Plate",                       //its name
       0.5*plate_sizeX, 0.5*plate_sizeY, 0.5*plate_sizeZ);
