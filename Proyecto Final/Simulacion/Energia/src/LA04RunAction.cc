  ...
  fGamma(0.)
{
  ...
  accumulableManager->RegisterAccumulable(fGamma);
}
...
void LA04RunAction::EndOfRunAction(const G4Run* run)
{
  ...
  // Añadimos los gammas que llegan al detector
  G4double Gammas = fGamma.GetValue();
...
// Agregamos la funcion para sumar los fGamma
void LA04RunAction::AddGamma(G4double gamma)
{
  fGamma += gamma;
}
